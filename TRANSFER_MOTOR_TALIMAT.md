...
## 6. Açık kararlar / Cursor'un uygulama sırasında kullanıcıya sorması gerekenler

- Garanti/İade ve Özel Sipariş akışlarında "bekleyen kabul" adımı mı olsun, yoksa bu iki modül için
  başlat+kabul aynı anda mı tetiklensin? (Bölüm 5 Faz 4'te işaretlendi.)
- `UtsDisFirma` tablosunda NG/ADESE/POTENTIAL şirketleri birbirine "dış firma" olarak tanımlı mı — değilse
  bu kayıtların oluşturulması gerekiyor, veri girişi kullanıcıdan istenmeli.
- Motor B'yi iki adımlı yapmak (Faz 3) mevcut mağaza iş akışını değiştiriyor — kullanıcıya/mağaza ekibine
  bu değişikliğin duyurulması gerekip gerekmediği ayrıca değerlendirilmeli.
